...
namespace stamp::serialize {
	...
	template<typename T>
	inline std::array<char, sizeof(T)> reverse_bytes_arr(const T& val) {
	...
	template<typename T>
	inline std::array<char, sizeof(T)> to_little_endian_arr(const T& val) {
	...
	template<typename T>
	inline T from_little_endian_arr(const std::array<char, sizeof(T)>& val) {
		if constexpr (std::endian::native == std::endian::little) {
