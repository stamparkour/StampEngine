...
void InitTimer() {
	...
	timerStart = largeInt.QuadPart * timerTickLength;
}
...
double getTime() {
	...
	return largeInt.QuadPart * timerTickLength - timerStart;
}
...
LRESULT Wndproc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {//WM_CLOSE
	...
	case WM_KEYDOWN:
	case WM_SYSKEYDOWN:
		GLCore::KeyDown(wParam, (lParam & (1 << 30)));
		break;
	case WM_KEYUP:
	case WM_SYSKEYUP:
		GLCore::KeyUp(wParam);
	...
	case WM_PAINT:
		break;
	case WM_SIZE:
		GLCore::Resize(getTime(), LOWORD(lParam), HIWORD(lParam));
...
int WINAPI wWinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR pCmdLine, int nCmdShow) {
	...
	while (IsWindow(hwnd)) {
		...
		std::thread update{ GLCore::Update, getTime() };
...
bool GLInput::IsKeyDown(int virtualKey) {
	return GetAsyncKeyState(virtualKey) < 0;
}

void GLCore::TerminateWindow() {
