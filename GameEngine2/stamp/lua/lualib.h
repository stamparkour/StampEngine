...
int lua_instance(lua_State* L, int index);
//returns the type of the id.
const char* lua_instancename(lua_State* L, int id);

int lua_getmaxmetaobjectid(lua_State* L);
// returns the id of the new metaobject, pushes new table onto stack
int luaL_newmetaobject(lua_State* L, const char* name);
// pushes the metaobject at id onto the stack, return 1 or 0 if it exists or not
int luaL_pushmetaobject(lua_State* L, int id);
// pushes the metaobject at id onto the stack, return 1 or 0 if it exists or not
int luaL_pushmetaobject(lua_State* L, const char* name);

//returns 1 if the object is a table, or userdata and has a metatable with __index
int lua_isindexable(lua_State* L, int index);
...
void luaL_openstamplibs(lua_State* L);
