...
export namespace render {
	...
	private:
		...
		virtual void setMaterial(MaterialBase& material) {
			glVertexAttribPointer(ATTRIBPOINTER_POSITION, 3, GL_FLOAT, GL_FALSE, sizeof(Point), (void*)offsetof(Point,pos));
			glVertexAttribPointer(ATTRIBPOINTER_NORMAL, 3, GL_FLOAT, GL_TRUE, sizeof(Point), (void*)offsetof(Point,normal));
			glVertexAttribPointer(ATTRIBPOINTER_UV, 2, GL_FLOAT, GL_FALSE, sizeof(Point), (void*)offsetof(Point,uv));
			glVertexAttribPointer(ATTRIBPOINTER_COLOR, 4, GL_FLOAT, GL_FALSE, sizeof(Point), (void*)offsetof(Point,color));
		}
	public:
		...
		static RawMeshP3NUC ParseStream_obj(std::istream& data) {
			STAMPERROR(data.fail(), "render::RawMeshP3NUC::ParseStream_obj - data stream is invalid.");
	...
	class Mesh final {
		...
		size_t count = 0;
		GLenum renderMode = 0;
		std::shared_ptr<RawMeshBase> meshBase{};
	public:
		Mesh() { }
		Mesh(const Mesh& other) = delete;
		Mesh(std::shared_ptr<RawMeshBase> meshBase, BufferUsageHint usageHint = BufferUsageHint::StaticDraw) {
		...
		void set(std::shared_ptr<RawMeshBase> meshBase, BufferUsageHint usageHint = BufferUsageHint::StaticDraw) {
	...
	protected:
		MaterialBase() { }
		MaterialBase(std::shared_ptr<ShaderProgramBase> shader, Mesh* mesh) {
			glGenVertexArrays(1, &vao);
			glBindVertexArray(vao);
			mesh->meshBase->setMaterial(*this);
			this->shader = shader;
		}
	...
	public:
		...
		void Update(int index = BLOCKBINDING_CAMERA) {
			...
			value.trans = transform.toMatrixInverse();
			value.cam = toMatrix();
	...
	public:
		...
		void Update(int index = BLOCKBINDING_CAMERAUI) {
			...
			Transform transform{};
			transform.position = { width / 2,height / 2, 0 };
			value.cam = toMatrix() * transform.toMatrixInverse();
			ubo.Set(&value, sizeof(cam_t), BufferUsageHint::DynamicDraw);
			ubo.BindBuffer();
		}
		//Uniform(location) camera {
